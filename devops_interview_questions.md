...
Сигналы:  
    ...
    - SIGKILL  
    - SIGTERM  
    - какой сигнал отправляется демону/сервису в случае systemctl stop daemon_name  
...
Сколько адресов в масках /25, /24, /21  
Адреса для локальных сетей(частный IP-адрес)  
Сколько портов доступно для одного адреса 
ARP  
MAC address  
...
Сколько ключей содержит симметричное и ассиметричное шифрование  
Какой ключ шифрует данные(открытй или закрытый)  
...
Обязаны ли все значения в Primary Key быть уникальными в рамках одной таблицы БД   
SQL:  
    - COUNT  
    - AVG  
    - SUM  
